...
from flask import Flask, render_template, session, url_for, request, redirect, send_from_directory
...
def content(id):
    if 'username' in session:
        ...
        cursor.close()
        conn.close()
        return render_template('content.html', data = content_data, username = username)
    else:
...
def write():
    if request.method == 'POST':
        if 'username' in session:
            ...
            conn = connectsql()
            cursor = conn.cursor()
            query = "INSERT INTO board (name, pass, title, content, file_name) VALUES (%s, %s, %s, %s, %s)"
